...
from models.vasnet.config import *
from models.vasnet.layer_norm import *
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, apperture=-1, ignore_itself=False, input_size=1024, output_size=1024):
        ...
        self.output_linear = nn.Linear(in_features=self.output_size,
                                       out_features=self.m, bias=False)
    ...
    def forward(self, x):
        ...
        logits = torch.bmm(Q, torch.transpose(K, 1, 2))
        logits = logits * 0.06
        ...
        att_weights_ = nn.functional.softmax(logits, dim=2)
...
class VASNet_batch(nn.Module):

    def __init__(self):
        super(VASNet_batch, self).__init__()
...
class VASNet_batch_proj(nn.Module):

    def __init__(self):
        super(VASNet_batch_proj, self).__init__()
