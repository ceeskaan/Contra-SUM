...
def halving_topk(frames, scores, k):
    n_frames = frames.shape[1]
    scores = scores.unsqueeze(-1)

    topk = TopKOperator()
    cfg = TopKConfig(input_len=n_frames,
                    pooled_len=k,
                    base=20,
                    sort_back=True 
                    )
    ...
    out_frames, _ = topk(frames, scores)
    return out_frames


"""
Courtesy of Michał Pietruszka and Łukasz Borchmann and Filip Graliński: https://github.com/applicaai/successive-halving-topk
"""
...
class TopKOperator(nn.Module):
    ...
    def our_topk(self, embs, scores):
        ...
        for layer in range(self.depth):
            ...
            #print(f'Splitted and sorted scores:{scores_converged}')
            if self.base > 0:
                ...
                #print('Weights after boosted softmax:')
                #print(scores_converged)
            else:
                raise ValueError
            #print(scores_before[:, pairs_idx] * scores_converged)
            ...
            #####################################################################################################
            # FIX: when tournament is over, and size is not the desired k, cut the results to the desired k
            if target_size < self.pooled_len:
                if self.sort_back:
                    original_order_scores = scores[:, pairs_idx[0].sort().indices]
                    original_order_embs = embs[:, pairs_idx[0].sort().indices]
                   
                    # Sort indices from original scores by value and select top k
                    indices = original_order_scores.clone().sort(descending=True).indices[:, :self.pooled_len]

                    # Sort top k indices
                    indices = indices.sort().values

                    scores = original_order_scores[:, indices[0]]
                    embs = original_order_embs[:, indices[0]]
                    

                    return embs, scores
                else:
                    embs = embs[:, :self.pooled_len]
                    scores = scores[:, :self.pooled_len]
                    return embs, scores
    ...
    def get_topk_pair_idx(self, scores):
        ...
        # FIX: if not divisible by 2, just truncate the last element###################################
