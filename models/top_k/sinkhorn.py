...
class TopK_stablized(torch.nn.Module):
    def __init__(self, k, epsilon=0.1, max_iter=200):
        ...
        self.register_buffer('anchors', torch.FloatTensor([0, 1]).view([1, 2, 1]))
        self.max_iter = max_iter
# TODO: Check if there are no bugs
        # if torch.cuda.is_available():
        #     self.anchors = self.anchors.cuda()
    ...
    def forward(self, scores):
        ...
        C = (scores - self.anchors)**2
        C = C / (C.max().detach())
        f = torch.zeros([bs, 1, n]).to(self.anchors.device)
        g = torch.zeros([bs, 2, 1]).to(self.anchors.device)
        mu = torch.ones([1, 1, n], requires_grad=False, device=self.anchors.device) / n
        nu = torch.Tensor([self.k / n, (n - self.k) / n]).view([1, 2, 1]).to(self.anchors.device)
